...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum InputMode {
    Normal, Edit, SearchInput, GotoInput, StringSearchInput, SaveConfirm, Help,
}

pub struct UndoEntry { pub offset: usize, pub old: u8, pub new: u8 }
...
impl App {
    pub fn new(file_size: usize, filename: String) -> Self {
        ...
        Self {
            ...
            current_pack: 0, scroll_top: 0,
            mode: DisplayMode::Ascii, input_mode: InputMode::Normal,
            filename, cursor_byte: 0, cursor_nibble: 0, dirty: false,
            undo_stack: Vec::new(), redo_stack: Vec::new(),
            search: None, search_active: false,
            pack_ranges: Vec::new(), pack_set: HashSet::new(),
            pack_match_idx: None, global_match_idx: None,
            input_buf: String::new(), input_prompt: String::new(),
            save_selected: false, sel_start: None, sel_end: None,
            help_scroll: 0, help_rect: None, cursor_focused: true,
        }
    }

    pub fn max_rows(&self, h: u16) -> usize { (h as usize).saturating_sub(4) }
    pub fn data_len(&self) -> usize { self.pack_size.min(self.file_size - self.current_pack * self.pack_size) }
    pub fn total_rows(&self) -> usize { (self.data_len() + 15) / 16 }
