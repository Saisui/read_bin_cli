...
// ─── byte-level pattern matcher (replaces regex for hex patterns) ──
#[derive(Clone, Copy)]
struct BytePat {
    mask: u8,
    val: u8,
...
fn compile_bytes_pattern(s: &str) -> Option<BytesPattern> {
    ...
    while i < ch.len() {
        if ch[i] == 'z' { pats.push(BytePat::any()); i += 1; }
            ...
            let (a, b) = (ch[i], ch[i + 1]);
            if a != 'x' && b != 'x' {
                pats.push(BytePat::exact(hv(a)? * 16 + hv(b)?));
            } else if a != 'x' && b == 'x' {
                pats.push(BytePat::hi_nibble(hv(a)?));
            } else if a == 'x' && b != 'x' {
                pats.push(BytePat::lo_nibble(hv(b)?));
            } else {
                pats.push(BytePat::any());
            }
            i += 2;
        }
    }
    ...
    pub fn extend(&mut self, mmap: &[u8], min_off: usize) -> bool {
        ...
        while start < self.file_size && (start < min_off + CHUNK || self.ranges.is_empty()) {
            ...
            match &self.needle {
                ...
                Needle::Re(re) => {
                    // regex-lite works on &str; use lossy conversion
