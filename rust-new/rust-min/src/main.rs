            ...
            match mode {
                ...
                InputMode::SearchInput => {
                    if let Some(kind) = search::parse_input(&buf) {
                        let (acc, label) = match kind {
                            search::SearchKind::Hex { bytes, label } => {
                                (search::Search::new_hex(bytes, app.pack_size, app.file_size, label.clone()), label)
                            }
                            search::SearchKind::Pat { pat, label } => {
                                (search::Search::new_pat(pat, app.pack_size, app.file_size, label.clone()), label)
                            }
                            search::SearchKind::Re { re, label } => {
                                (search::Search::new_re(re, app.pack_size, app.file_size, label.clone()), label)
                            }
                        ...
                        app.apply_search(acc, &**mmap, th);
                        let _ = label; // used for search label already
                    }
                }
                _ => {}
            }
            ...
            for seg in &segs {
                match seg {
                    utf8::Segment::Char { pos, ch, len } => {
                        ...
                        if cur_type == prev_type { same_count += 1; }
                        else { same_count = 0; prev_type = cur_type; }
