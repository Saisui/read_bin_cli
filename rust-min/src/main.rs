        ...
        match evt {
            ...
            Event::Key(key) => {
                if key.modifiers.contains(KeyModifiers::CONTROL) {
                    match key.code {
                        KeyCode::Char('z') => { app.undo(&mut *mmap); continue; }
                        KeyCode::Char('y') => { app.redo(&mut *mmap); continue; }
                        KeyCode::Char('q') => {
                            if app.dirty {
                ...
                if key.modifiers.contains(KeyModifiers::ALT) {
                    match key.code {
                        KeyCode::Char('j') => { app.sel_start = Some(app.cursor_byte); continue; }
                        KeyCode::Char('k') => { app.sel_end = Some(app.cursor_byte); continue; }
                        KeyCode::Char('m') => { app.mode = app.mode.next(); continue; }
                        _ => {}
                    }
                }
...
fn handle_save(app: &mut App, code: KeyCode, do_break: &mut bool) {
    match code {
        ...
        KeyCode::Char('y') | KeyCode::Char('Y') => { app.save_selected = true; *do_break = true; }
        KeyCode::Char('n') | KeyCode::Char('N') => { app.save_selected = false; *do_break = true; }
        KeyCode::Enter | KeyCode::Char(' ') => *do_break = true,
        KeyCode::Esc => { app.save_selected = false; *do_break = true; }
        _ => {}
    }
}
                ...
                InputMode::SearchInput => {
                    if let Some(kind) = search::parse_input(&buf) {
                        let acc = match kind {
                            ...
                            search::SearchKind::Re { re, label } => {
                                search::Search::new_re(re, app.pack_size, app.file_size, label)
                            }
                            search::SearchKind::Str { bytes, label } => {
                                search::Search::new_hex(bytes, app.pack_size, app.file_size, label)
                            }
                        };
                        app.apply_search(acc, &**mmap, th);
...
fn handle_edit(app: &mut App, code: KeyCode, mmap: &mut MmapMut, th: u16) {
    match code {
        ...
        KeyCode::Left => {
            if app.mode == DisplayMode::Hex {
                if app.cursor_nibble == 0 {
                    if app.cursor_byte > 0 { app.cursor_byte -= 1; app.cursor_nibble = 1; }
                } else { app.cursor_nibble = 0; }
            } else if app.cursor_byte > 0 { app.cursor_byte -= 1; }
            app.ensure_cursor_visible(th);
        }
        KeyCode::Right => {
            if app.mode == DisplayMode::Hex {
                if app.cursor_nibble == 0 { app.cursor_nibble = 1; }
                else if app.cursor_byte + 1 < app.file_size { app.cursor_byte += 1; app.cursor_nibble = 0; }
            } else if app.cursor_byte + 1 < app.file_size { app.cursor_byte += 1; }
            app.ensure_cursor_visible(th);
        }
        KeyCode::Up => { app.cursor_byte = app.cursor_byte.saturating_sub(16); app.ensure_cursor_visible(th); }
        KeyCode::Down => {
            if app.cursor_byte + 16 < app.file_size { app.cursor_byte += 16; }
            app.ensure_cursor_visible(th);
        }
        KeyCode::PageUp => {
        ...
        KeyCode::Enter => { if app.mode == DisplayMode::Ascii { app.edit_char(&mut *mmap, '\n'); } app.ensure_cursor_visible(th); }
        KeyCode::Tab => { if app.mode == DisplayMode::Ascii { app.edit_char(&mut *mmap, '\t'); } app.ensure_cursor_visible(th); }
        KeyCode::Char(c) => {
            if app.mode == DisplayMode::Hex {
                if c.is_ascii_hexdigit() { app.edit_hex(&mut *mmap, c); }
            } else { app.edit_char(&mut *mmap, c); }
            app.ensure_cursor_visible(th);
        }
        _ => {}
    }
...
fn handle_normal(
    ...
    if code == KeyCode::Esc {
        if app.search_active { app.clear_search(); }
        return;
    }
    match code {
        KeyCode::Char('q') => {
            if app.dirty { app.input_mode = InputMode::SaveConfirm; app.save_selected = true; }
            else { *do_break = true; }
        }
        ...
        KeyCode::Up | KeyCode::Char('k') => {
            if app.search_active { nav_pack_match(app, -1); }
            else { app.scroll_top = app.scroll_top.saturating_sub(1); }
        }
        KeyCode::Down | KeyCode::Char('j') => {
            if app.search_active { nav_pack_match(app, 1); }
            else {
                let tr = app.total_rows();
                if app.scroll_top + max_rows < tr { app.scroll_top += 1; }
            }
        }
        KeyCode::Left | KeyCode::Char('h') => {
            if app.search_active { app.prev_global(); }
            else if app.current_pack > 0 { app.current_pack -= 1; app.scroll_top = 0; }
        }
        KeyCode::Right | KeyCode::Char('l') => {
            if app.search_active { app.next_global(&**mmap); }
            else if app.current_pack + 1 < app.total_packs { app.current_pack += 1; app.scroll_top = 0; }
        }
        KeyCode::Char('K') => { app.scroll_top = app.scroll_top.saturating_sub(max_rows); }
        KeyCode::Char('J') => {
            let tr = app.total_rows();
        ...
        KeyCode::Char('H') => {
            ...
            if app.search_active {
                let off = target * app.pack_size;
                if let Some(ref mut s) = app.search {
                    if let Some(idx) = s.find_after(&**mmap, off) { app.jump_global(idx); }
                }
            } else { app.current_pack = target; app.scroll_top = 0; }
        }
        KeyCode::Char('L') => {
        ...
        KeyCode::Home => {
            if app.search_active {
                if let Some(ref mut s) = app.search {
                    if let Some(idx) = s.find_after(&**mmap, 0) { app.jump_global(idx); }
                }
            } else { app.current_pack = 0; app.scroll_top = 0; }
        }
            ...
            if app.search_active {
                ...
                if let Some(ref mut s) = app.search {
                    if let Some(idx) = s.find_after(&**mmap, min) {
                        if idx != app.global_match_idx.unwrap_or(usize::MAX) || min <= s.ranges[idx].0 {
                            app.jump_global(idx);
                        }
                    }
                }
            } else { app.current_pack = app.current_pack.saturating_sub(256); app.scroll_top = 0; }
        }
        KeyCode::Char('P') | KeyCode::Char('p') => {
            if app.search_active {
                ...
                if let Some(ref mut s) = app.search {
                    if let Some(idx) = s.find_after(&**mmap, min) { app.jump_global(idx); }
                }
            } else { app.current_pack = (app.current_pack + 256).min(app.total_packs - 1); app.scroll_top = 0; }
        }
        _ => {}
    }
}
...
fn nav_pack_match(app: &mut App, delta: i32) {
    if app.pack_ranges.is_empty() { return; }
    ...
    if let Some(ref s) = app.search {
        if let Some(gi) = s.idx_for_offset(ms) { app.global_match_idx = Some(gi); }
    }
...
fn byte_type_group(b: u8, mode: DisplayMode) -> u8 {
    match mode {
        DisplayMode::Ascii => {
            if b == 0 { 1 } else if b == 0x0d { 2 } else if b == 10 || b == 0x20 { 3 }
            else if b == 0x1b || (0x01..=0x1f).contains(&b) { 4 }
            else if (0x21..=0x7e).contains(&b) { 5 } else if (0x80..=0xbf).contains(&b) { 6 } else { 7 }
        }
        DisplayMode::Hex => {
            if (0x20..=0x7e).contains(&b) { 1 } else if b == 0 { 2 } else if b == 0x0d { 3 }
            else if b == 10 { 4 } else if b == 0x1b || (0x01..=0x1f).contains(&b) { 5 }
            else if (0x80..=0xbf).contains(&b) { 6 } else { 7 }
        }
        _ => 0,
    }
}
...
fn byte_style(b: u8, mode: DisplayMode) -> Style {
    match mode {
        DisplayMode::Ascii => {
            if b == 0 { sp(1) } else if b == 0x0d { sp(2) } else if b == 10 || b == 0x20 { sp(5) }
            else if b == 0x1b || (0x01..=0x1f).contains(&b) { sp(4) }
            else if (0x21..=0x7e).contains(&b) { sp(5) } else if (0x80..=0xbf).contains(&b) { sp(6) }
            else { sp(8) }
        }
        DisplayMode::Hex => {
            if (0x20..=0x7e).contains(&b) { sp(10) } else if b == 0 { sp(1) } else if b == 0x0d { sp(2) }
            else if b == 10 { sp(5) } else if b == 0x1b || (0x01..=0x1f).contains(&b) { sp(4) }
            else if (0x80..=0xbf).contains(&b) { sp(6) } else { sp(8) }
        }
...
fn char_type_group(ch: char) -> u8 {
    let cp = ch as u32;
    if cp >= 0x21 && cp <= 0x7e { 1 }
    else if ch == '\n' || ch == '\r' || ch == '\t' { 2 }
    else if cp < 0x20 { 3 }
    else if cp >= 0x4E00 && cp <= 0x9FFF { 4 }
    else if cp >= 0xAC00 && cp <= 0xD7AF { 5 }
    else if cp >= 0x3000 && cp <= 0x30FF { 6 }
    else if cp >= 0x2000 && cp <= 0x206F { 7 }
    else { 8 }
}

fn utf8_char_style(ch: char) -> Style {
    let cp = ch as u32;
    if cp >= 0x21 && cp <= 0x7e { sp(5) }
    else if ch == '\n' || ch == '\r' || ch == '\t' { sp(6) }
    else if cp < 0x20 { sp(4) }
    else if cp >= 0x80 && cp <= 0xBF { sp(3) }
    else if cp >= 0x2000 && cp <= 0x206F { sp(8) }
    else if cp >= 0x4E00 && cp <= 0x9FFF { sp(2) }
    else if cp >= 0xAC00 && cp <= 0xD7AF { sp(6) }
    else if cp >= 0x3000 && cp <= 0x30FF { sp(8) }
    else { sp(10) }
}
...
fn dim_color(c: Color) -> Color {
    let (r, g, b) = match c {
        ...
        Color::Red => (205, 0, 0), Color::Green => (0, 205, 0), Color::Yellow => (205, 205, 0),
        Color::Blue => (0, 0, 205), Color::Magenta => (205, 0, 205), Color::Cyan => (0, 205, 205),
        Color::White => (205, 205, 205), Color::DarkGray => (105, 105, 105), Color::Black => (0, 0, 0),
        _ => (128, 128, 128),
    };
...
fn resolve(app: &App, off: usize, base: Style, mr: Option<(usize, usize)>) -> Style {
    if app.cursor_focused && app.cursor_byte == off { return sp(16); }
    if let Some((ms, me)) = mr { if ms <= off && off < me { return sp(13); } }
    if app.search_active && app.pack_set.contains(&off) { return sp(12); }
    if let (Some(a), Some(b)) = (app.sel_start, app.sel_end) {
        let (lo, hi) = if a <= b { (a, b) } else { (b, a) };
        if off >= lo && off <= hi { return if off == a { sp(18) } else { sp(17) }; }
    }
    if app.input_mode == InputMode::Edit { return dim_style(base); }
    base
}
...
fn build_lines<'a>(app: &App, data_full: &[u8], area: Rect) -> Vec<Line<'a>> {
    ...
    lines.push(Line::from(Span::raw(format!("{}  ({})", app.filename, App::format_size(app.file_size)))));
    let pack_str = format!("{:x} / {:x}", app.current_pack + 1, app.total_packs);
    let mut mode_str = app.mode.label().to_string();
    if app.input_mode == InputMode::Edit { mode_str.push_str(" [EDIT]"); }
    ...
    for r in start..end {
        ...
        if app.mode == DisplayMode::Utf8 {
                ...
                match seg {
                    utf8::Segment::Char { pos, ch, len } => {
                        ...
                        let dc = match *ch {
                            ...
                            _ => { let s: String = ch.to_string(); if dw == 1 { format!("{} ", s) } else { s } }
                        };
                        let cur_type = char_type_group(*ch);
                        if cur_type == prev_type { same_count += 1; } else { same_count = 0; prev_type = cur_type; }
                        let dim = same_count % 2 == 1;
                        let base = if (*ch as u32) < 0x20 { byte_style(*ch as u8, DisplayMode::Ascii) } else { utf8_char_style(*ch) };
                        let sty = resolve(app, go, base, mr);
                        let final_sty = if dim { dim_bg_10pct(sty) } else { sty };
                        spans.push(Span::styled(dc, final_sty));
                        for ci in 1..*len {
                            if pos + ci >= rem { cross_row_tail = *len - ci; break; }
                            let cgo = base_off + off + pos + ci;
                            spans.push(Span::styled("··".to_string(), resolve(app, cgo, sp(3), mr)));
                        }
                    }
                    utf8::Segment::Invalid { pos } => {
                        ...
                        let go = base_off + bo;
                        let b = data[bo];
                        spans.push(Span::styled(format!("{:02x}", b), resolve(app, go, utf8_cls_style(utf8::byte_class(b)), mr)));
                    }
                }
            }
        } else {
            ...
            let mut same_count: usize = 0;
            for i in 0..16 {
                if off + i >= data.len() { spans.push(Span::raw("  ".to_string())); continue; }
                let b = data[off + i];
    ...
    let text = match app.input_mode {
        InputMode::Edit => {
            ...
            return f.render_widget(
                Paragraph::new(Span::raw(format!("{}{}", match app.mode {
                    DisplayMode::Ascii => "[EDIT ASCII]", DisplayMode::Utf8 => "[EDIT UTF8]", DisplayMode::Hex => "[EDIT HEX]",
                }, byte_info))),
        ...
        InputMode::Normal => {
            if app.search_active {
                if let Some(ref s) = app.search {
                    ...
                    if disp.len() > 24 { disp.truncate(24); disp.push_str("..."); }
                    return f.render_widget(
                        Paragraph::new(Span::styled(format!("Search: {} [{}/{}{}]  ↑↓:in-pack ←→:global ESC:clear", disp, cur, total, plus), sp(5))),
                        Rect::new(0, area.height - 1, area.width, 1),
                    );
                }
            }
            let dirty = if app.dirty { " [MODIFIED]" } else { "" };
            let hex_w = if app.file_size <= 0xff { 2 } else if app.file_size <= 0xffff { 4 } else if app.file_size <= 0xffffff { 6 } else { 8 };
            return f.render_widget(
                Paragraph::new(Span::styled(format!("{}{}  @{:0width$x}  pack {}/{}  Ctrl+H:help", app.mode.label(), dirty, app.cursor_byte, app.current_pack + 1, app.total_packs, width = hex_w), sp(5))),
                Rect::new(0, area.height - 1, area.width, 1),
            );
        }
    };
    f.render_widget(Paragraph::new(Span::styled(text, sp(5))), Rect::new(0, area.height - 1, area.width, 1));
}

fn draw_help(f: &mut ratatui::Frame, app: &App, area: Rect) {
    let lines_text = [
        "=== HELP ===", "",
        "Navigation:", "  ↑↓ / jk    Scroll one line", "  ←→ / hl    Prev/Next pack",
        "  J / K       Scroll one screen", "  H / L       Jump ±16 packs",
        "  PGUP/PGDN   Scroll half screen", "  HOME        First pack", "  O / P       ±1MB area",
        "  g/Ctrl+G    Go to offset (hex)", "",
        "Search:", "  f           Hex / regex search", "  F           Plain string search",
        "  ↑↓ / ←→    Navigate matches", "  ESC         Clear search", "",
        "Edit:", "  i           Enter edit mode", "  ESC         Exit edit mode",
        "  ←→↑↓       Move cursor", "  PGUP/PGDN   Scroll page",
        "  0-9a-f      Edit nibble (hex)", "  any char    Edit byte (ascii/utf8)", "",
        "Selection:", "  Alt+J       Selection start (bright)", "  Alt+K       Selection end", "",
        "Other:", "  m / Alt+M   Toggle mode", "  Ctrl+H / ?  This help",
        "  Ctrl+Z / Y  Undo / Redo", "  Ctrl+Q / q  Quit",
        "  Mouse click Move cursor", "  Scroll wheel Scroll page",
    ];
    ...
    let help: Vec<Line> = visible.iter().map(|l| Line::from(Span::styled(l.to_string(), Style::default().fg(Color::White)))).collect();
    ...
    if max_scroll > 0 && inner_h > 0 {
        ...
        for row in 0..sb_h {
            ...
            let style = if row == thumb_pos { Style::default().fg(Color::Cyan).bg(Color::Rgb(20, 20, 40)) }
                else { Style::default().fg(Color::DarkGray).bg(Color::Rgb(20, 20, 40)) };
            f.render_widget(Paragraph::new(Span::styled(ch, style)), Rect::new(sb_x, sb_y + row, 1, 1));
        }
    }
}
...
fn draw_save_dialog(f: &mut ratatui::Frame, app: &App, area: Rect) {
    ...
    f.render_widget(Paragraph::new(Span::raw("Save changes before quitting?")), Rect::new(dx + 2, dy + 1, dw - 4, 1));
    let ys = if app.save_selected { Style::default().fg(Color::Black).bg(Color::White) } else { Style::default() };
    let ns = if !app.save_selected { Style::default().fg(Color::Black).bg(Color::White) } else { Style::default() };
    f.render_widget(
        Paragraph::new(Line::from(vec![Span::styled(" Yes ", ys), Span::raw("   "), Span::styled(" No ", ns)])),
